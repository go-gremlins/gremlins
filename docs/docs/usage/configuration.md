...
5. Excluded files are set by default to empty list, which means no files skipped except tests. To suppress
   mutations at a finer granularity (single statement, block, or file) without editing the configuration, see the
   [`//nomutant` directive](nomutant-directive.md).
